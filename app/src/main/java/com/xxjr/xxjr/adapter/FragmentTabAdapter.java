...
public class FragmentTabAdapter implements RadioGroup.OnCheckedChangeListener {
    ...
    public void onCheckedChanged(RadioGroup radioGroup, int checkedId) {
        for (int i = 0; i < rgs.getChildCount(); i++) {
            if ((checkedId == R.id.tab_rb_b || checkedId == R.id.tab_rb_c || checkedId == R.id.tab_rb_d) && MyApplication.uid == null) {
                Intent intent = new Intent(fragmentActivity, LoginActivity.class);
                fragmentActivity.startActivityForResult(intent, LOGINMSG);
                return;
            }
...
//        }
     ...
     * @param position
     */
    public void commitFragment(int position) {
        FragmentTransaction ft = fragmentActivity.getSupportFragmentManager().beginTransaction();
        ft.add(fragmentContentId, fragments.get(position));
        ft.commitAllowingStateLoss();
    }
