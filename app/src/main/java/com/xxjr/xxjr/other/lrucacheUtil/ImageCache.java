package com.xxjr.xxjr.other.lrucacheUtil;
...
public class ImageCache {
	 ...
	 * @param imageView
     */
	/*private void setImageWidthAndHeighBitmap(Bitmap bitmap,ImageView imageView){
		int height = bitmap.getHeight();
		int width = bitmap.getWidth();

		CommonMath.measureView(imageView);
		...
		int mearureHeight = (int) (( height*measuredWidth)*size/width);
		layoutParams.height = mearureHeight;
	...
	private synchronized void addTaskToWaitQueue(ImageDownloadTask task,
		...
		DebugLog.e("download", "addTaskToWaitQueue");
	}
	...
	private synchronized void removeTaskFromWaitQueue(ImageDownloadTask task) {
		...
		DebugLog.e("download", "removeTaskFromWaitQueue");
	}
	...
	private synchronized void addTaskToLoadQueue(ImageDownloadTask task) {
		...
		DebugLog.e("download", "addTaskToLoadQueue");
	}
	...
	private synchronized void removeTaskFromLoadQueue(ImageDownloadTask task) {
		...
		DebugLog.e("download", "removeTaskFromLoadQueue");
	}
	...
	private synchronized void addTaskToQueue(ImageDownloadTask task,
		...
		if (loadingQueue.size() <= MAX_THREAD_NUM) {
			...
			task.execute();
			DebugLog.e("download", "addTaskToQueue addTaskToLoadQueue");
		} else {
			addTaskToWaitQueue(task, priority);
			DebugLog.e("download", "addTaskToQueue addTaskToWaitQueue");
		}
	}
	...
	private synchronized void waitToLoadStrategy() {
		...
		DebugLog.e("download", "waitToLoadStrategy index = " + index);
	...
	class ImageDownloadTask extends AsyncTask<String, Void, Bitmap> {
		...
		protected Bitmap doInBackground(String... params) {
			...
			DebugLog.e("tag", "imageUrl = " + imageUrl);
			try {
				// 生成图片URL对应的key
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			DebugLog.e("tag", "onPostExecute = " + imageUrl);
