package com.xxjr.xxjr.other.seekbar;
...
import com.xxjr.xxjr.utils.DensityUtil;
...
public class MaterialRangeSlider extends View {
    ...
    private static final int DEFAULT_TOUCH_TARGET_SIZE = Math.round(
            (float) (MyApplication.density * 40));
    ...
    private boolean isSingleCircle = false;//  标示是否只有一个圆圈
    public ObjectAnimator minAnimator;
    public ObjectAnimator maxAnimator;
    ...
    public void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            isSingleCircle = styledAttrs.getBoolean(R.styleable.MaterialRangeSlider_isSingleCircle, false);
...
//        colorControlNormal = materialStyledAttrs.getColor(0, appcompatMaterialStyledAttrs.getColor(0,getResources().getColor(android.R.color.holo_blue_dark) ));
//        colorControlHighlight = materialStyledAttrs.getColor(1, appcompatMaterialStyledAttrs.getColor(1,getResources().getColor(android.R.color.black) ));
    ...
    private void getDefaultMeasurements() {
        pressedRadius = Math.round(MyApplication.density * DEFAULT_PRESSED_RADIUS);
        unpressedRadius = Math.round(MyApplication.density * DEFAULT_UNPRESSED_RADIUS);
        insideRangeLineStrokeWidth = Math.round(MyApplication.density * DEFAULT_INSIDE_RANGE_STROKE_WIDTH);
        outsideRangeLineStrokeWidth = Math.round(MyApplication.density * DEFAULT_OUTSIDE_RANGE_STROKE_WIDTH);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int desiredHeight = (int) (30*MyApplication.density);
        ...
        HORIZONTAL_PADDING = (int) pressedRadius;
        lineLength = (int) (width - HORIZONTAL_PADDING * 2);
        midY = height / 2;


        lineStartX = (int) (HORIZONTAL_PADDING);
        lineEndX = (int) (lineLength + HORIZONTAL_PADDING);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (getParent() != null&&(isTouchingMaxTarget(actionIndex,event) || isTouchingMinTarget(actionIndex,event))) {
            getParent().requestDisallowInterceptTouchEvent(true);
        }

        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                if (isSingleCircle) {//  单圆圈的情况
                    if (lastTouchedMin) {
    ...
    public void isSingleCircle(boolean isSingleCircle) {
